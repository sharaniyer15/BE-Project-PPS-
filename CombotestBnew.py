...
data = pd.read_csv("G:/BE Project Divided Data/Updated CSV/SSC-HSC_CE-IT_11-12.csv", sep = ",")
#print(data.shape)
X =  data.iloc[:,1:9]
#print(X.dtypes)
y = data.iloc[:,9]
...
data1 = pd.read_csv("G:/BE Project Divided Data/Updated CSV/SSC-HSC_CE-IT_12-13.csv", sep = ",")
X1 = data1.iloc[:,1:9]
y1 = data1.iloc[:,9]
Y1 = y1.to_frame()
...
data2 = pd.read_csv("G:/BE Project Divided Data/Updated CSV/SSC-HSC_CE-IT_13-14.csv", sep = ",")
X2 = data2.iloc[:,1:9]
y2 = data2.iloc[:,9]
Y2 = y2.to_frame()
...
data2 = pd.read_csv("G:/BE Project Divided Data/Updated CSV/SSC-HSC_CE-IT_14-15.csv", sep = ",")
X3 = data2.iloc[:,1:9]
y3 = data2.iloc[:,9]
Y3 = y3.to_frame()
...
for max_depth in range(1,7):
      for max_leaf_nodes in range(3,8):
          for k in [3,4,5,6,7]:
               X_feature= SelectKBest(mutual_info_classif, k=k).fit_transform(Xt,Y2)
               X_feature=pd.DataFrame(X_feature)
               X_feature1= SelectKBest(mutual_info_classif, k=k).fit_transform(Xt2,Y3)
               X_feature1=pd.DataFrame(X_feature1)
...
#        print("For i and j = %s,%s"%(i,j))
    ...
    #        print(type(Y_Train))
        ...
        ###        print('Y Pred : ', Y_pred)
                    ...
                    print(" Accuracy is : ", AS(Y_Test,Y_pred)*100, ' ',max_depth, ' ',max_leaf_nodes, ' ',k)
        ##            print('Y Pred : ', pred)
